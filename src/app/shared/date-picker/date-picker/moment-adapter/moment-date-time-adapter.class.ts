...
import _moment, { Moment } from 'moment/moment';
...
export class MomentDateTimeAdapter extends DateTimeAdapter<Moment> {
    ...
    public createDate( year: number, month: number, date: number, hours: number = 0, minutes: number = 0, seconds: number = 0 ): Moment {
        ...
        if (!this.checkHoursMinutesExists(hours, minutes, seconds) && window['projectStartTime']) {
            hours = window['projectStartTime']?.hours;
            minutes = window['projectStartTime']?.minutes;
        }
        seconds = 0;
    ...
    public now(): Moment {
        ...
        if (window['projectStartTime']) {

            const projectDate = new Date().setHours(window['projectStartTime']?.hours, window['projectStartTime']?.minutes, window['projectStartTime']?.seconds);
